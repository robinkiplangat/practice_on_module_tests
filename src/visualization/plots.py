...
def get_plot_for_best_fit_line(slope, intercept, x_array, y_array, title):
    ...
    if x_array_length != y_array_length:
        raise RuntimeError(
            "Arguments x_array and y_array should have same length. "
            "But x_array has length {0} and y_array has length {1}".format(
                x_array_length, y_array_length
            )
        )
    fig, ax = plt.subplots()
